...
#  الإعدادات
# ══════════════════════════════════════════════════════════════
TOKEN     = os.environ.get("TOKEN", "8741347112:AAErqD32d4GoHHr2dO8dD9exV3jL3bjDbIA")
ADMIN_ID  = int(os.environ.get("ADMIN_ID", "8567682905"))
ADMIN_IDS = [int(x) for x in os.environ.get("ADMIN_IDS", "8567682905,8206539702").split(",")]
...
# ══════════════════════════════════════════════════════════════
#  قاعدة البيانات — تعريف المسار مبكراً
# ══════════════════════════════════════════════════════════════
...
def load_db():
    default = {
        ...
        "settings":  {
            ...
            "credits_per_upload":   10,
            "credits_for_referral": 50,
            "vip_min_credits":      500,
            "welcome_credits":      20,
            "maintenance":          False,
            "auto_vip":             True,
...
def cmd_start(m):
    ...
    if len(parts) > 1 and parts[1].startswith("ref_"):
        ...
        if referrer_uid and referrer_uid != uid and not db["users"][uid].get("referred_by"):
            ...
            save()
            add_credits(referrer_uid, db["settings"].get("credits_for_referral",50), "دعوة صديق 👥")
            add_credits(uid, 10, "انضممت بدعوة 🎁")
    ...
    bot.send_message(m.chat.id,
        ...
        f"💡 `/ref` — رابط دعوتك | `/credits` — رصيدك",
...
def cmd_credits(m):
    ...
    bot.reply_to(m,
        ...
        f"صلاحيتك: `{role.upper()}`\n\n"
        f"{'🌟 أنت VIP بالفعل!' if role in [ROLE_VIP,ROLE_ADMIN,ROLE_OWNER] else f'للترقية لـ VIP: `{max(0,vip_min-credits)}` نقطة متبقي'}\n"
...
def handle_upload(m):
    ...
    def deploy():
        try:
            ...
            # ── حفظ الملف ────────────────────────────
            path = f"ELITE_HOST/{fname}"
            # احتفظ بنسخة قديمة لو الملف موجود
            ...
            save()
            # نقطة لكل رفع
            pts = db["settings"].get("credits_per_upload",10)
            add_credits(uid, pts, f"رفع {fname} 📤")
            check_vip_upgrade(uid)
...
def main_handler(m):
    ...
    if text == "🔗 رابط الدعوة" or text == "/ref":
        code = db["users"][uid].get("referral_code","")
        try:
            bot_info = bot.get_me()
            link = f"https://t.me/{bot_info.username}?start=ref_{code}"
        ...
        bot.reply_to(m,
            f"🔗 رابط دعوتك:\n{link}\n\n"
            f"👥 دعوت: {total} شخص\n"
            f"💎 نقطة لكل دعوة: {db['settings'].get('credits_for_referral',50)}"); return
    ...
    if text == "📊 إحصائياتي":
        ...
        bot.reply_to(m,
            f"📊 *إحصائياتك*\n━━━━━━━━━━━━━━━━━\n"
            f"📂 ملفاتك: `{len(uid_files)}`\n"
            f"✅ شغالة: `{active}`\n"
            f"❌ متوقفة: `{len(uid_files)-active}`\n"
            f"📦 إجمالي الحجم: `{total_kb} KB`\n"
            f"📤 إجمالي الرفعات: `{uploads}`\n"
            f"🆔 ID: `{uid}`",
